"""Generate private GEE exports for the frozen 12-month Presto primary grid."""
...
DEFAULT_OUTPUT = ROOT / "outputs/eofm/ee/presto_monthly_code_editor_private.js"
...
def main() -> None:
    ...
    if grid["timestep_count"] != 12 or grid["month_argument_zero_based"] != 0:
        raise ValueError("Monthly Presto grid contract changed")
    ...
    xy = frame.iloc[selected_start:selected_stop][["longitude", "latitude"]].astype(float).values.tolist()
    windows = [[w["tag"], w["start_inclusive"], w["end_exclusive"]] for w in grid["windows"]]
    s2_bands = list(ee_config["sentinel_2_bands"])
    feature_bands: list[str] = []
    for tag, _, _ in windows:
        feature_bands += [f"s2_{b.lower()}_{tag}" for b in s2_bands]
        feature_bands += [f"s2_valid_{tag}", f"s2_count_{tag}", f"s1_vv_{tag}", f"s1_vh_{tag}", f"s1_valid_{tag}", f"s1_count_{tag}"]
    lines = [
        f"var xy={compact(xy)};", f"var selectedStart={selected_start};",
        ...
        f"var windows={compact(windows)};", f"var s2Bands={compact(s2_bands)};",
        f"var excluded={compact(ee_config['sentinel_2_scl_excluded'])};", f"var missing={float(ee_config['missing_value'])};",
        ...
        "var clearS2=s2.map(maskS2);var s2Native=clearS2.select(s2Bands).median();var s2Names=s2Bands.map(function(b){return 's2_'+b.toLowerCase()+'_'+tag;});"
        "var s2Valid=s2Native.select('B2').mask().rename('s2_valid_'+tag).unmask(0).toUint8();var s2Count=clearS2.select('B2').count().rename('s2_count_'+tag).unmask(0).toUint16();"
        "stack=stack.addBands(s2Native.rename(s2Names).unmask(missing)).addBands(s2Valid).addBands(s2Count);"
