...
| M1b | **FTW + label-independent Dynamic World land-cover gate** | `outputs/phase4_chongming_staged/products/M1b_ftw_dynamicworld_gated_probability.tif` (M1 ∩ Dynamic World crop/flooded-veg) |
...
| Product | Precision | Recall | F1 | IoU | Spatial retention | Pred. rice (ha) | Ref. rice (ha) | Area Δ (ha) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
...
> *Spatial retention* (the "Coverage" column) is the fraction of the common
> evaluation grid that a product's spatial priors still map. Mode A metrics are
> computed over the **entire** common grid, so this column is **not**
> evaluation coverage — it is retained spatial support (28.1%, 15.7%, 8.7% for
> M1, M1b, M2 respectively).

### 4.2 Mode B — Conditional retained-coverage (quality-at-coverage)

| Product | Precision | Recall | F1 | IoU | Spatial retention | Pred. rice (ha) | Ref. rice retained (ha) | Area Δ (ha) |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
...
- **M0** over-predicts rice massively relative to the weak reference: 619,672
  false positives vs 157,322 true positives → precision **0.202**. Under the
  natural low rice prevalence of the region, the frozen Jiangxi-trained source
  model produces many false-positive rice predictions, which drives precision
  down to 0.202 and therefore F1 down to 0.329 — not a recall failure (M0 recall
  is in fact high at 0.882).
...
- **M1b** adds the label-independent Dynamic World land-cover gate: precision **0.49**, F1
  **0.53**, recall 0.57.
- **M1b** has the highest full-grid precision (**0.488**); **M2 / M2_QA**
  precision is 0.455 and they have the **lowest recall (0.30)** and only
  **8.7% spatial retention** — parcel aggregation concentrates on coherent field
  objects and leaves most of the grid (including much reference rice) unmapped.
  Full-grid F1 is **0.36**.
...
- M1 reaches **0.60**, M1b **0.65**, M2/M2_QA **0.62** retained-coverage F1,
  ...
  where it chooses to map*, **not** how much of the region it covers. Spatial
  retention is 28.1% (M1), 15.7% (M1b), 8.7% (M2). They must **not** be cited as
...
- Paired block analysis over the common grid (full-grid F1 vs M0):

  The common evaluation grid contains **36 blocks**, but paired ΔF1 is computed
  only where F1 is defined for **both** M0 and the compared product. The
  evaluable block counts are **n_evaluable = 35 (M1), 32 (M1b), 31 (M2/M2_QA)**:
...
| `results/tables/wall_to_wall_block_metrics.csv` | Per-block full-grid F1 over the common 36-block grid (paired ΔF1 computed where defined) |
...
| `assets/figures/wall_to_wall_area_coverage_tradeoff.png` | Fig 2: predicted vs reference area + spatial retention |
